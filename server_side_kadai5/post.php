...
if(isset($_SESSION['POST']['url'])){
    $title =$_SESSION['POST']['url'];
}
if (isset( $_SESSION['POST']['type'])){
    $content = $_SESSION['POST']['type'];
}
?>
...
<body>
    ...
    <!-- // もしURLパラメータがある場合 issetでif文とendifをすることで、HTMLは入れられるようになり、エラーの場合のメッセージを入れられる -->
<?php if(isset($_GET[('error')])):?>  
 <p class='text-danger'>記入内容を入力してください。</P>
<?php endif ?>

<!-- enctypeを入れないと画像が入れられない　おまじないだそう -->
    <form method="POST" action="confirm.php" enctype="multipart/form-data"> 
        <div class="mb-3">
            <label for="title" class="form-label">タイトル</label>
            <input type="text" class="form-control" name="name" id="title" aria-describedby="title" value="<?= $name ?>">
